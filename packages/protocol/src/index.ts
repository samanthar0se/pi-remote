...
export const reviewStartedSchema = z.object({
  type: z.literal("review_started"), reviewId: z.string().min(1), kind: z.enum(["plan", "code"]), url: z.string().url(),
}).strict();
export const reviewFinishedSchema = z.object({
  type: z.literal("review_finished"), reviewId: z.string().min(1), kind: z.enum(["plan", "code"]), approved: z.boolean().optional(), error: z.string().optional(),
}).strict();
export const errorMessageSchema = z.object({
  type: z.literal("error"), code: z.string(), message: z.string(), requestId: z.string().optional(),
}).strict();
